...
def preprocess_LISA(LISA_path: str, proportion: float = 1.0, testset: bool = False) -> Dataset:
...
def preprocess_sim(sim_path: str, proportion: float = 1.0, testset: bool = False) -> Dataset:
...
def preprocess_bosch(bosch_path: str, proportion: float = 1.0, testset: bool = False) -> Dataset:
...
def preprocess_mturk(mturk_path: str, proportion: float = 1.0, testset: bool = False) -> Dataset:
...
def preprocess_custom(custom_path: str, proportion: float = 1.0, testset: bool = False) -> Dataset:
    """Preprocess and generate data for our custom dataset at the given path.
...
def preprocess_cities(cities_path: str, proportion: float = 1.0, testset: bool = False) -> Dataset:
    """Preprocess and generate data for our custom dataset at the given path.
        ...
        with open(os.path.join(cities_path, f)) as csv_file:
            ...
            for row in csv_reader:
                if id_index == 0:
                    ...
                    image_path = os.path.abspath(
                        os.path.join(cities_path, "images_new", "{}.png".format(img_id))
                    )
...
def create_testset(name: str,
    ...
    for key, val in images.items():
        ...
        random.seed(config.RAND_SEED)
        inv_indices = sorted(random.sample([i for i in range(int(len(val)))], total))
