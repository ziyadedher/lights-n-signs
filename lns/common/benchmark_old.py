...
if __name__ == '__main__':
    ...
    bosch = Preprocessor.preprocess("Bosch", proportion=0.9, testset=True)
    cities = Preprocessor.preprocess("cities", proportion=0.9, testset=True)
    lights = Preprocessor.preprocess("lights", proportion=0.9, testset=True)

    full = bosch + cities + lights
    full = full.merge_classes({
        "green": ["GreenLeft", "Green", "GreenRight", "GreenStraight", "GreenStraightRight", "GreenStraightLeft", "Green traffic light"],
        "red": ["Yellow", "RedLeft", "Red", "RedRight", "RedStraight", "RedStraightLeft", "Red traffic light", "Yellow traffic light"],
        "off": ["off"]
    })

    import sys
    from lns_squeezedet.model import SqueezeDetModel
    model = SqueezeDetModel(sys.argv[0], ["green", "red", "off"])

    benchmark_model(full, model)
